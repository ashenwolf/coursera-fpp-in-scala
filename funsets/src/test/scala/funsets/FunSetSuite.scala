...
class FunSetSuite extends munit.FunSuite:
  ...
  test("forall test") {
    new TestSetsRange:
      assert(!forall(s1, el => el > 0), "Union 1")
      assert(forall(s1, el => el > -20), "Union 2")
  }

  test("exists test") {
    new TestSetsRange:
      assert(exists(s1, el => el < 0), "Union 1")
      assert(!exists(s1, el => el < -100), "Union 2")
  }
