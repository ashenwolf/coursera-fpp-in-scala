...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet =
    val newAcc = if acc.retweets > elem.retweets then acc else elem
    left.mostRetweetedAcc(right.mostRetweetedAcc(newAcc))

  def descendingByRetweet: TweetList = 
    val maxRetweets = this.mostRetweeted
    Cons(maxRetweets, this.remove(maxRetweets).descendingByRetweet)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter((tweet: Tweet) => google.exists((keyword: String) => tweet.text.contains(keyword)))
  lazy val appleTweets: TweetSet = allTweets.filter((tweet: Tweet) => apple.exists((keyword: String) => tweet.text.contains(keyword)))
